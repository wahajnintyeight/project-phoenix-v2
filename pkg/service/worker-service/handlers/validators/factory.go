...
func (f *ValidatorFactory) registerValidators() {
	...
	f.Register(NewOpenAICompatibleValidator(model.ProviderMistral, "https://api.mistral.ai/v1/chat/completions", "mistral-large-latest", "max_tokens", f.debugMode))
	f.Register(NewOpenAICompatibleValidator(model.ProviderBytePlus, "https://ark.ap-southeast.bytepluses.com/api/v3/chat/completions", "dola-seed-2-1-turbo", "max_tokens", f.debugMode))
}
